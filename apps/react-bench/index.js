		...
		render() {
			...
			const kButton = React.createElement(
				'button',
				{
					id: 'create1000',
					key: '1000',
					onClick: () => this.setState({ tableRows: 1000, counter: (this.state.counter += 1) }),
				},
				'Create K'
			);
			...
			const tenKButton = React.createElement(
				'button',
				{
					id: 'create10000',
					key: '10000',
					onClick: () => this.setState({ tableRows: 10000, counter: (this.state.counter += 1) }),
				},
				'Create 10K'
			);
			...
			const clearButton = React.createElement(
				'button',
				{
					id: 'clear',
					key: '0',
					onClick: () => this.setState({ tableRows: 0, counter: (this.state.counter += 1) }),
				},
				'Clear'
			);
			const header = React.createElement('header', { key: 'header' }, [h1, kButton, tenKButton, clearButton]);
			...
			if (tableRows > 0) {
				...
				for (let i = 1; i <= tableRows; i++) {
					...
					const wordStr = `${words[wordIdx % 12]} ${words[wordIdx % 13]} ${words[wordIdx % 14]}`;
